...
class RepositorySplitter:
        ...
        for action in actions:
            ...
            if action_type == "move_files":
                ...
                if package_name and not self.dry_run:
                    ...
                    plugin_name = (
                        package_name.replace("rompy_", "") if is_plugin else None
                    )
    ...
    def _correct_imports(self, target_dir: str, package_type: str, target_package: str):
        ...
        # Find all relevant files (Python and documentation) for import correction
        file_exts = [".py", ".rst", ".md", ".txt"]
        files_to_process = set()
        for root, dirs, files in os.walk(target_dir):
            # Only skip directories named exactly '.git' or 'notebooks'
            ...
            for file in files:
                ...
                # Include documentation files (.rst, .md, .txt) anywhere, and .py files in docs/
    ...
    def _apply_cookiecutter_template(
        self,
        target_dir: str,
        template_repo: str,
        ...
        if not COOKIECUTTER_AVAILABLE:
            logger.error(
                "Cookiecutter is not available. Install with: pip install cookiecutter"
            )
            return
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            try:
                ...
                self._inject_dependencies_from_template_context(
                    target_dir, template_context
                )
        ...
        if package_name == "rompy":
            ...
            if (
                ...
                if "rompy.source" in monorepo_entry_points:
                    ...
                    logger.info(
                        f"[inject_deps] Injected rompy.source entry-points from monorepo into split package."
                    )
            ...
            if "rompy.config" in data["project"].get("entry-points", {}):
                ...
                logger.info(
                    f"[inject_deps] Set rompy.config entry-point to only base=rompy.core.config:BaseConfig for core package."
                )
            ...
            for file in files:
                ...
                if merge_strategy == "replace":
                    ...
                    if file in cookiecutter_priority:
                        should_copy = True
                        logger.debug(
                            f"Taking infrastructure file from cookiecutter: {file}"
                        )

                    # NEVER overwrite existing source files anywhere, especially .py files
                    elif (
                        is_source_file and os.path.exists(dst_file)
                    ) or file_ext == ".py":
                        should_copy = False
                        logger.debug(
                            f"Preserving existing source file: {rel_path}/{file}"
                        )
    ...
    def _get_import_corrections(self, package_type: str, target_package: str) -> list:
        ...
        if package_type == "core":
            ...
            corrections.extend(
                [
                    # Replace all occurrences of 'rompy.swan.' with 'rompy_swan.'
                    (r"rompy\.swan\.", f"{target_package}."),
                    # Replace all occurrences of 'rompy.swan' (as a word) with 'rompy_swan'
                    (r"rompy\.swan\b", f"{target_package}"),
                    # Convert other rompy imports to rompy-core (leave as-is for now)
            ...
            # For rompy-schism, robustly convert schism-specific imports and references
            # Use a single-pass regex with longest patterns first, and word boundaries
            corrections.extend(
                [
                    # Replace 'rompy.schism.' (dotted import or attribute)
                    (r"\brompy\.schism\.", f"{target_package}."),
                    # Replace 'rompy.schism' as a whole word (not followed by a dot)
                    (r"\brompy\.schism\b", f"{target_package}"),
                    # (Optional) Replace in doc references like ``rompy.schism``
                    (r"``rompy\.schism``", f"``{target_package}``"),
                    # (Optional) Replace in doc references like :mod:`rompy.schism`
                    (r":mod:`rompy\.schism`", f":mod:`{target_package}`"),
                    # (Optional) Replace in doc references like :py:mod:`rompy.schism`
                    (r":py:mod:`rompy\.schism`", f":py:mod:`{target_package}`"),
                ]
            )
            # Leave other rompy imports as-is for now (core, swan, etc.)
    ...
    def _apply_import_corrections(self, file_path: str, corrections: list) -> bool:
        """
        Apply import corrections to a single file (code or docs) using robust, single-pass regex.
        ...
        try:
            ...
            # Sort corrections by pattern length (longest first) to avoid partial matches
            corrections_sorted = sorted(
                corrections, key=lambda x: len(x[0]), reverse=True
            )
            for pattern, replacement in corrections_sorted:
                # Use re.DOTALL to match across lines if needed, and re.MULTILINE for line-anchored patterns
