...
def find_generated_files(staging_dir: Path) -> dict:
    ...
    for file_path in staging_dir.rglob("*"):
        if file_path.is_file():
            ...
            elif 'sflux' in name or name.endswith('.nc') and any(x in name for x in ['air', 'wind', 'prc']):
...
def create_plots_from_real_data(files: dict, model_run, save_plots: bool = False, output_dir: Path = None):
    ...
    try:
        ...
        if files['atmospheric']:
            ...
            try:
                fig, ax = plotter.plot_atmospheric_data(figsize=(14, 10))
                if save_plots and plot_dir:
        ...
        if files['boundary_data']:
            ...
            for i, boundary_file in enumerate(files['boundary_data'][:3]):  # Limit to first 3
                try:
        ...
        try:
            fig, axes = plotter.plot_comprehensive_overview(figsize=(20, 16))
            if save_plots and plot_dir:
        ...
        try:
            fig, axes = plotter.plot_grid_analysis_overview(figsize=(16, 12))
            if save_plots and plot_dir:
        ...
        try:
            fig, axes = plotter.plot_data_analysis_overview(figsize=(16, 12))
            if save_plots and plot_dir:
        ...
        try:
            ...
            logger.info(f"Validation Summary: {status_counts['PASS']} PASS, "
...
def main():
    ...
    parser = argparse.ArgumentParser(description="SCHISM Real Data Model Run and Plotting Demo")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("./schism_demo_output"),
    ...
    try:
        ...
        if not args.plot_only:
            ...
            if not model_run_dir.exists():
                logger.error("No existing model data found. Run without --plot-only first.")
                return 1
