...
class PlotConfig(RompyBaseModel):
    ...
    figsize: Tuple[float, float] = Field(
        default=(12, 8), description="Figure size in inches (width, height)"
    )
    dpi: int = Field(default=100, description="Figure resolution in dots per inch")

    # Colormap parameters
    cmap: str = Field(default="viridis", description="Colormap for data visualization")
    ...
    add_coastlines: bool = Field(
        default=True, description="Whether to add coastlines to the plot"
    )
    add_gridlines: bool = Field(
        default=True, description="Whether to add coordinate gridlines"
    )
    projection: str = Field(
        default="PlateCarree", description="Cartopy projection for geographic plots"
    )

    # Grid parameters
    show_grid: bool = Field(
        default=True, description="Whether to show the computational grid"
    )
    grid_alpha: float = Field(default=0.3, description="Transparency of grid lines")
    grid_color: str = Field(default="gray", description="Color of grid lines")

    # Boundary parameters
    show_boundaries: bool = Field(
        default=True, description="Whether to show model boundaries"
    )
    ...
    boundary_linewidth: float = Field(
        default=2.0, description="Line width for boundary plotting"
    )

    # Colorbar parameters
    add_colorbar: bool = Field(default=True, description="Whether to add a colorbar")
    colorbar_label: Optional[str] = Field(
        default=None, description="Label for the colorbar"
    )
    colorbar_orientation: str = Field(
        default="vertical", description="Orientation of the colorbar"
    )

    # Title and labels
    title: Optional[str] = Field(default=None, description="Plot title")
    xlabel: Optional[str] = Field(default="Longitude", description="X-axis label")
    ylabel: Optional[str] = Field(default="Latitude", description="Y-axis label")
...
class BasePlotter(ABC):
    ...
    def create_figure(
        self, ax: Optional[Axes] = None, use_cartopy: bool = True, **kwargs
    ) -> Tuple[Figure, Axes]:
        """
        ...
        **kwargs : dict
        ...
        if use_cartopy and self.plot_config.add_coastlines:
            try:
                ...
                logger.warning(
                    f"Cartopy not available or failed: {e}, using regular matplotlib axes"
                )
    ...
    def add_colorbar(
        self, fig: Figure, ax: Axes, mappable: Any, label: Optional[str] = None
    ) -> Any:
        """
...
class PlotValidator:
    ...
    def validate_dataset(
        ds: xr.Dataset, required_vars: Optional[List[str]] = None
    ) -> bool:
        """
    ...
    def validate_coordinates(
        ds: xr.Dataset, required_coords: Optional[List[str]] = None
    ) -> bool:
        """
        ...
        if required_coords:
            missing_coords = [
                coord for coord in required_coords if coord not in ds.coords
            ]
            if missing_coords:
