...
class AnimationPlotter(BasePlotter):
    ...
    def animate_grid_data(
        ...
        **kwargs : dict
        ...
        # Initialize plot elements
        self._setup_animation_plot(ax, ds, variable, show_grid=show_grid, **kwargs)
    ...
    def _animate_boundary_frame(
        ...
        **kwargs
        ...
        # Create scatter plot for boundary data
        if hasattr(self, '_get_grid') and self._get_grid():
            grid = self._get_grid()
            # Reason: Use grid coordinates for spatial boundary visualization
            try:
                # Assume boundary node indices are available
                boundary_nodes = np.arange(len(data.values))
                if hasattr(grid, 'x') and hasattr(grid, 'y'):
                    x_coords = grid.x[boundary_nodes]
                    y_coords = grid.y[boundary_nodes]
        ...
        **kwargs
        ...
        if hasattr(self, '_get_grid') and self._get_grid():
            ...
            try:
                ...
                if hasattr(grid, 'x') and hasattr(grid, 'y'):
                    im = ax.scatter(
                        grid.x, grid.y, c=data.values,
                        cmap=self.animation_config.cmap, s=20, **plot_kwargs
                    )
                    
                    # Add grid overlay if requested
                    if show_grid and hasattr(grid, 'triangles'):
                        ax.triplot(grid.x, grid.y, grid.triangles, 
        ...
        if requested_var in ds.data_vars:
            ...
            for var_name, var in ds.data_vars.items():
                if 'time' in var.dims and var_name not in ['time_step', 'nOpenBndNodes', 'nLevels', 'nComponents', 'one']:
                    return var_name
            # Fallback to time_series if nothing else found
            return 'time_series'
