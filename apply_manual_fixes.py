...
def apply_core_exports_fix():
    ...
    try:
        ...
        content = content.replace('# from .config import BaseConfig', 'from .config import BaseConfig')
        content = content.replace('# from .data import DataBlob, DataGrid, DataPoint', 'from .data import DataBlob, DataGrid, DataPoint')
        content = content.replace('# from .filters import *', 'from .filters import *')
        content = content.replace('# from .grid import BaseGrid, RegularGrid', 'from .grid import BaseGrid, RegularGrid')
        content = content.replace('# from .spectrum import LogFrequency', 'from .spectrum import LogFrequency')
        content = content.replace('# from .time import TimeRange', 'from .time import TimeRange')
...
def apply_swan_components_fix():
    ...
    split_dir = Path("../split-repos").resolve()
    components_init = split_dir / "rompy-swan" / "src" / "rompy_swan" / "components" / "__init__.py"
...
def apply_schism_circular_import_fix():
    ...
    split_dir = Path("../split-repos").resolve()
    namelists_init = split_dir / "rompy-schism" / "src" / "rompy_schism" / "namelists" / "__init__.py"
...
def main():
    ...
    if successful_fixes == len(fixes):
        ...
        print("\nNext steps:")
        print("1. Run: python setup_split_testing.py --split-repos-dir ../split-repos --package rompy-core")
        print("2. Run: python setup_split_testing.py --split-repos-dir ../split-repos --package rompy-swan")
        print("3. Run: python setup_split_testing.py --split-repos-dir ../split-repos --package rompy-schism")
        print("4. Test: python run_split_tests.py --split-repos-dir ../split-repos --package rompy-core")
